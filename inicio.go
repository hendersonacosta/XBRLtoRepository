...
func main() {
	...
	arquivoxml, err := os.Open(`C:\Users\manue\go\src\XMLtoRelacional\arquivos\instancia.xml`)
	if err != nil {
		fmt.Println("Não carregou o arquivo")
	}

	byteValueXML, _ := ioutil.ReadAll(arquivoxml)

	defer arquivoxml.Close()

	var processed xbrl.XBRL

	xml.Unmarshal(byteValueXML, &processed)
